...
def resolve_local_call_edges(conn: sqlite3.Connection, repo_id: str) -> int:
    target_rows = list(
        conn.execute(
            """
            SELECT id, name, qname, kind
            FROM nodes
    ...
    by_qname = {row["qname"]: row for row in target_rows}
    ...
    with conn:
        conn.execute(
            """
            DELETE FROM edges
            WHERE repo_id = ? AND kind = 'CALLS' AND confidence = 'resolved'
            """,
            (repo_id,),
        )
        external_edges = list(
        ...
        for edge in external_edges:
            ...
            target_id = resolve_call_target(call_name, target_rows, by_qname, by_name)
            if not target_id or target_id == edge["src"]:
                continue
            result = conn.execute(
                """
                ...
                """,
                (repo_id, edge["owner_path"], edge["src"], target_id, call_name),
            )
...
def resolve_call_target(
    ...
    if not call_name:
        return None
    exact = by_qname.get(call_name)
    if exact:
        return exact["id"]
    if "." in call_name:
        suffix = f".{call_name}"
        suffix_matches = [row for row in target_rows if row["qname"].endswith(suffix)]
